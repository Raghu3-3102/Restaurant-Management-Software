...
const Admin = () => {
  ...
  const getCorrect = async (foods)=>{
    try {
      ...
      const response = await axios.post('/api/user/employe-approved',foods)
  ...
  const getAllfood = async ()=>{
    ...
    try {
      dispatch(showLoading())
     const response = await axios.get('/api/user/get-request-for-employe')
     if (response.data.success) {
   ...
   const columns = [
    ...
    //   title: "Status",
    //   dataIndex: "isManager",
    //   render:isManager => (isManager ? 'approved' : 'pending')
    // }, 
    {
      title: "applicentid",
      dataIndex: "applicentid",
    },
